...
function driverMobileCard(world,data,driver,team,contract,pot){
  return `<article class="card driver-mobile-card" style="--team-color:${team?.color??"#687384"}">
    <div class="driver-mobile-head">${avatar(driver,"large")}<div><span class="pill ${contract.tone}">${contract.label}</span><h3>${driver.countryCode??""} ${driver.name}</h3><p>${team?.shortName??"Libre"} · ${driver.age} años · #${driver.number??"—"}</p></div><strong>${driverOverall(driver)} Rating</strong></div>
...
function detail(world, data, driver) {
  ...
  <section class="driver-detail-hero" style="--team-color:${team?.color ?? "#687384"}">${avatar(driver, "xl")}<div class="driver-identity">${badge(contract.label, contract.tone)}<h2>${driver.name}</h2><p>#${driver.number} · ${driver.countryCode} · ${driver.age} años · ${teamChip(team)}</p>${potentialBadge(driver, team)}</div><div class="overall-ring xl"><strong>${driverOverall(driver)}</strong><span>Rating</span></div></section>
...
export function renderDriverScreen(world, data, selectedDriverId = null) {
  ...
  <div class="driver-directory">${sorted.map(driver => {
    ...
    return `<button class="card directory-card" style="--team-color:${team?.color ?? "#687384"}" data-action="view-driver" data-driver="${driver.id}">${avatar(driver, "large")}<div class="directory-info"><span class="pill ${contract.tone}">${contract.label}</span><h3>${driver.name}</h3><p>${driver.countryCode} · ${driver.age} años · ${academy?.name ?? "Independiente"}</p><span>${teamChip(team)}</span></div><div class="directory-stats"><div><strong>${driverOverall(driver)}</strong><small>Rating</small></div><div class="stars-cell"><strong>${pot.rating}</strong><small>${pot.label}</small></div><div><strong>${driver.reputation}</strong><small>REP.</small></div></div></button>`;
  }).join("")}</div>`;
}
