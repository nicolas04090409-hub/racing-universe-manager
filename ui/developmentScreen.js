import { componentLabels, developmentCapacity, projectQuote } from "../engine/developmentEngine.js";
...
function teamDevelopmentRow(world, team, position) {
  ...
  const completed = [...(world.projectHistory ?? []), ...(world.developmentProjects ?? [])].filter(p => p.teamId === team.id && p.season === world.currentSeason && p.type !== "facility" && p.status === "completed");
  ...
  return { position, team, active, completed, investment, delta, areas, trend };
}
...
export function renderDevelopmentScreen(world) {
  ...
  const activeProjects = projects.filter(p => ["planned", "active"].includes(p.status));
  const components = Object.entries(team.carDevelopment ?? {});
  const capacity = developmentCapacity(team);
  ...
  const lastResult = team.lastDevelopmentResult;

  return `${pageHead("Ingeniería / Desarrollo", `Componentes modulares · Presupuesto ${formatMoneyCompact(team.budget)}`)}
  <article class="card notice"><strong>Qué estás viendo:</strong> invertís en el coche y comparás tu evolución real contra rivales. Ahora los proyectos son más cortos, la capacidad depende del tamaño del equipo y el impacto se refleja en el rating global.</article>
  ${lastResult ? `<article class="card result-banner ${lastResult.status === "completed" ? "good" : "bad"}"><span class="metric-label">ÚLTIMO RESULTADO DE I+D</span><h3>${lastResult.message}</h3></article>` : ""}
  <div class="development-overview card"><div><span class="kicker">PAQUETE ${world.currentSeason}</span><h2>Nivel global ${team.carPerformance}</h2><p>Inicio ${playerRow?.delta.base.carPerformance ?? team.carPerformance} · evolución ${sign(playerRow?.delta.global ?? 0)} · ${playerRow?.trend ?? "Estable"}</p><p>Capacidad: ${activeProjects.length}/${capacity} proyectos activos.</p></div><div class="tech-orbit"><strong>${team.carPerformance}</strong><span>TECH</span></div></div>
  <div class="mobile-card-list development-mobile-cards">
    ...
    <article class="card"><span class="metric-label">Capacidad I+D</span><h3>${activeProjects.length}/${capacity}</h3><p>Top: hasta 3–4, medios: 2, chicos: 1–2.</p></article>
  ...
  ${pageHead("Proyectos activos", `${activeProjects.length} en curso`)}
  <div class="grid two">${projects.map(p => `<article class="card"><div class="card-title-row"><div><span class="pill ${p.status === "completed" ? "good" : p.status === "failed" ? "bad" : "info"}">${p.status}</span><h3>${componentLabels[p.component]}</h3><p>Riesgo ${p.risk}% · Ganancia esperada +${p.expectedGainMin}-${p.expectedGainMax}</p></div><strong>${Math.round(p.progress)}%</strong></div>${progress("Progreso", p.progress, "blue")}<p>Inversión: ${formatMoneyCompact(p.investment)}</p>${p.status==="completed"?`<p class="positive-text">Resultado: +${p.gain} componente · global ${p.beforePerformance ?? "?"} → ${p.afterPerformance ?? "?"}</p>`:""}</article>`).join("") || `<article class="card notice">No hay proyectos activos. Elegí un componente para iniciar I+D.</article>`}</div>
  ...
  <div class="grid three development-grid">${components.map(([component, value]) => {
    ...
    const disabledReason = team.budget < quote.cost ? "Presupuesto insuficiente" : value >= 100 ? "Componente al máximo" : active ? "Ya hay proyecto activo" : activeProjects.length >= capacity ? "Capacidad de I+D llena" : "";
  ...
  ${developmentBars(rows.slice(0,5))}
...
function developmentBars(rows){
  return `<article class="card chart-card"><span class="metric-label">EVOLUCIÓN DEL COCHE</span><h3>Inicio de temporada → actual</h3><div class="bar-chart">${rows.map(row=>{const start=row.delta.base.carPerformance??row.team.carPerformance,now=row.team.carPerformance??0;return`<div class="bar-row"><span>${row.team.shortName??row.team.name}</span><div class="bar-track"><em style="--start:${start}%;--now:${now}%"></em></div><strong>${start} → ${now}</strong></div>`}).join("")}</div></article>`;
}
