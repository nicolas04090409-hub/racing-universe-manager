...
function qualifyingBlock(world,weekend){
  ...
  <div class="qualy-compact-list">${s.results.slice(0,12).map(r=>`<article class="card"><b>P${r.position}</b><span>${compactName(getDriver(world,r.driverId)?.name)}</span><strong>${getTeam(world,r.teamId)?.shortName??"—"}</strong></article>`).join("")}</div>
...
function raceBlock(world,weekend){
  ...
  return `${pageHead("Carrera",`${weatherLabel(s.weather)} · ${s.events.length} eventos`)}<div class="grid two">${driverCards}</div><div class="grid two"><article class="card"><h3>Eventos</h3>${s.events.slice(0,12).map(e=>`<div class="standing-line"><b>${e.segment}</b><span>${e.message??e.description??e.type}</span><strong>${e.type}</strong></div>`).join("")||`<p>Sin eventos relevantes.</p>`}</article><article class="card"><h3>Top 10</h3>${s.results.slice(0,10).map(r=>`<div class="standing-line"><b>P${r.position}</b><span>${getDriver(world,r.driverId)?.name}</span><strong>${r.points} pts</strong></div>`).join("")}</article></div>`;
}
