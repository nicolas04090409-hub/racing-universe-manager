...
import { getDriver,getTeam,money,pageHead,teamChip,userTeam } from "./helpers.js";
...
export function renderNegotiationsScreen(world){
  ...
  <div class="table-wrap"><table class="data-table negotiation-table"><thead><tr><th>Piloto</th><th>Equipo interesado</th><th>Oferta</th><th>Estado</th><th>Respuesta esperada</th><th>Acciones</th></tr></thead><tbody>${items.map(n=>{
    const driver=getDriver(world,n.driverId),offer=n.offer??{},interestedTeam=getTeam(world,n.teamId),state=statusLabel[n.status]??n.status;
    return`<tr><td><button class="table-link" data-action="view-driver" data-driver="${driver?.id}">${driver?.name??"Piloto"}</button></td><td>${teamChip(interestedTeam)}${n.type==="ai"?`<small class="muted">IA</small>`:`<small class="muted">Jugador</small>`}</td><td><strong>${money(offer.salary??0)}</strong><small class="muted">${offer.duration??1} años · ${roleLabels[offer.role]??offer.role} · bono ${money(offer.signingBonus??0)}</small></td><td><span class="pill ${statusTone[n.status]??""}">${state}</span><small class="muted">${n.interest?.label??""} ${n.interest?.score!=null?`${n.interest.score}/100`:""}</small></td><td>Ronda ${n.responseDueRound??"—"}<small class="muted">${n.history?.at(-1)?.message??""}</small></td><td><div class="actions">${n.type==="player"&&n.status==="accepted"?`<button class="button small primary" data-action="finalize-offer" data-negotiation="${n.id}">Firmar</button>`:""}${n.type==="player"&&["sent","counter","waiting"].includes(n.status)?`<button class="button small danger" data-action="withdraw-offer" data-negotiation="${n.id}">Retirar</button>`:""}${n.type==="player"&&n.status==="counter"?`<button class="button small" data-action="improve-offer" data-negotiation="${n.id}">Aceptar contraoferta</button>`:""}</div></td></tr>`;
