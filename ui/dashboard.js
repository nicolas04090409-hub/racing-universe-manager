...
function latestRaceCard(world,team,latest){
  ...
  return `<article class="card latest-race-card"><span class="metric-label">ÃšLTIMA CARRERA</span><h3>R${latest.round} Â· ${latest.weather??"Clima variable"}</h3>${player.map(r=>`<div class="standing-line"><b>${getDriver(world,r.driverId)?.name?.split(" ").at(-1)??r.driverId}</b><span>P${r.position}</span><strong>${r.points??0} pts</strong></div>`).join("")||`<p class="muted">Sin pilotos del equipo en el resultado.</p>`}<p>+${points} puntos Â· Constructores P${teamRow?.position??"â€”"}</p><button class="button full" data-action="go" data-screen="race">Ver resultado</button></article>`;
}
...
export function renderDashboard(world,data){
  ...
  const stage=weekend?.stage==="setup"?"Viernes Â· Setup":weekend?.stage==="practice"?"Viernes Â· PrÃ¡cticas":weekend?.stage==="qualifying"?"SÃ¡bado Â· Qualy":weekend?.stage==="race"?"Domingo Â· Carrera":weekend?.stage==="post_race"?"Post carrera":"Previo al GP";
  const alerts=paddockAlerts(world,summary,drivers);

  return`<section class="dashboard-hero"><div><span class="kicker">${summary.category.name} Â· ${summary.category.realEquivalent} Â· ${world.currentSeason}</span><h2>${summary.nextRace?summary.nextRace.name:"Temporada completada"}</h2><p>${summary.nextRace?`${summary.nextRace.location}, ${summary.nextRace.country}: ${summary.nextRace.laps} vueltas Â· ronda ${summary.round}/${summary.totalRounds}.`:"El calendario de tu categorÃ­a estÃ¡ completo."}</p></div><div class="hero-rank"><span>CONSTRUCTORES</span><strong>${summary.position??"â€”"}</strong><small>POSICIÃ“N ACTUAL</small></div></section>
  ...
  <section class="paddock-hub">
    <article class="card paddock-hero" style="--team-color:${team.color}">
      ...
      <h3>${circuit?`${circuit.name} Â· ${stage}`:"Temporada completada"}</h3>
    ...
    <div class="mobile-dashboard-cards paddock-cards">
      ...
      ${actionCard("PrÃ³ximo evento",circuit?`${circuit.name} Â· ${stage} Â· lluvia ${circuit.rainProbability}%`:"Calendario completado","race-weekend","Ir al Race Weekend")}
      ${actionCard("Equipo",`Constructores P${summary.position??"â€”"} Â· coche ${summary.carRating} Â· moral ${Math.round(drivers.reduce((a,d)=>a+(d.morale??70),0)/Math.max(1,drivers.length))} Â· conf. ${summary.reliability}`,"team","Ver equipo")}
      <article class="card mobile-action-card"><h3>Pilotos</h3>${drivers.map(d=>{const contract=contractStatus(d,world.currentSeason);return`<div class="standing-line"><b>P${summary.driverStandings?.find?.(r=>r.driverId===d.id)?.position??"â€”"}</b><span>${d.name} Â· moral ${d.morale}</span><strong>${contract.label}</strong></div>`}).join("")}<button type="button" class="button full" data-action="go" data-screen="drivers">Ver pilotos</button></article>
      <article class="card mobile-action-card"><h3>IngenierÃ­a</h3><p>${summary.projects.total?summary.projects.label:"Sin proyectos activos"}.</p>${[...summary.projects.development,...summary.projects.facilities].slice(0,1).map(p=>`${progress("Progreso",p.progress,"blue")}<p>${p.weeksRemaining??"?"} semana(s) restantes</p>`).join("")}<button type="button" class="button full" data-action="go" data-screen="development">Ver desarrollo</button></article>
      ${actionCard("Mercado",`${stateText(expiring.length,"contrato por vencer","contratos por vencer")} Â· ${(world.negotiations??[]).filter(n=>["sent","counter","accepted","waiting"].includes(n.status)).length} negociaciÃ³n(es) activas.`,"market","Ver mercado")}
  ...
  <div class="grid four dashboard-metrics">${metricCard("Equipo",team.shortName,team.name)}${metricCard("Balance",money(summary.balance),summary.balance>=0?"ProyecciÃ³n positiva":"Revisar gastos",summary.balance>=0?"good":"bad")}${metricCard("Coche",summary.carRating,`Confiabilidad ${summary.reliability}`)}${metricCard("Proyectos",summary.projects.total,summary.projects.label)}</div>
  <div class="dashboard-grid"><div class="dashboard-main stack">
    <article class="card race-preview"><div class="card-title-row"><div><div class="metric-label">PRÃ“XIMA CARRERA</div><h2>${summary.nextRace?.name??"Calendario completado"}</h2></div><span class="round-badge">${summary.nextRace?.countryCode??"â€”"}</span></div>${summary.nextRace?`<div class="race-facts"><div><span>Tipo</span><strong>${summary.nextRace.streetCircuit?"Callejero":"Permanente"}</strong></div><div><span>Adelantamiento</span><strong>${summary.nextRace.overtaking}</strong></div><div><span>NeumÃ¡ticos</span><strong>${summary.nextRace.tyreWear}</strong></div><div><span>Lluvia</span><strong>${summary.nextRace.rainProbability}%</strong></div></div><div class="actions"><button type="button" class="button primary" data-action="go" data-screen="race-weekend">Abrir Race Weekend</button><button type="button" class="button" data-action="simulate-world">Simular mundo</button></div>`:""}</article>
    ...
    <article class="card"><div class="metric-label">RENDIMIENTO DEL COCHE</div><div class="grid two compact-grid"><div>${progress("Global",summary.carRating)}${progress("AerodinÃ¡mica",team.aerodynamics)}${progress("Motor",team.engine)}</div><div>${progress("Chasis",team.chassis)}${progress("Confiabilidad",team.reliability,"green")}${progress("Setup",team.setupQuality??team.strategy,"blue")}</div></div><div class="actions"><button type="button" class="button small" data-action="go" data-screen="development">Abrir IngenierÃ­a</button><button type="button" class="button small" data-action="go" data-screen="facilities">Instalaciones</button></div></article>
    ...
    <article class="card"><div class="metric-label">PROYECTOS ACTIVOS</div>${summary.projects.total?`<p>${summary.projects.label}</p>${[...summary.projects.development,...summary.projects.facilities].slice(0,4).map(p=>`<div class="standing-line"><b>${Math.round(p.progress)}%</b><span>${p.component??p.facility}</span><strong>${p.status}</strong></div>`).join("")}`:`<p>No hay proyectos activos.</p>`}<button type="button" class="text-button full" data-action="go" data-screen="development">Gestionar proyectos â†’</button></article>
    ${latestRaceCard(world,team,latest)}
