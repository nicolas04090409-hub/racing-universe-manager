...
export function renderDashboard(world,data){
  ...
  <section class="paddock-hub">
    <article class="card paddock-hero" style="--team-color:${team.color}">
      ...
      <p>${summary.category.name} · Temp. ${world.currentSeason} · R${Math.min(summary.round,summary.totalRounds)}/${summary.totalRounds}</p>
  ...
  <div class="dashboard-grid"><div class="dashboard-main stack">
    ...
    <article class="card"><div class="metric-label">PILOTOS PRINCIPALES</div><div class="driver-line-grid">${drivers.map(d=>{const contract=contractStatus(d,world.currentSeason),pot=potentialSummary(d,team);return`<button type="button" class="driver-line" data-action="view-driver" data-driver="${d.id}">${avatar(d)}<div><strong>${d.name}</strong><span>${d.countryCode} · ${d.age} años · ${contract.label}</span></div><div class="driver-line-stat"><b>${driverOverall(d)}</b><small>CA</small></div><div class="driver-line-stat stars-mini"><b>${pot.rating}</b><small>${pot.label}</small></div></button>`}).join("")}</div></article>
    ...
    <article class="card"><div class="metric-label">FINANZAS RÁPIDAS</div><div class="finance-strip"><div><span>Caja</span><strong>${money(summary.budget)}</strong></div><div><span>Balance</span><strong class="${summary.balance>=0?"good":"bad"}">${money(summary.balance)}</strong></div></div><button type="button" class="text-button full" data-action="go" data-screen="finances">Ver detalle financiero →</button></article>
