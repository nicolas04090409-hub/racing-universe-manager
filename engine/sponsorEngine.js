...
export function evaluateSponsorInterest(sponsor, team, world) {
  let score=35+(team.reputation??50)*.35+(team.finances?.projectedBalance>0?8:-8);
  ...
  for(const req of sponsor.requirements??[]){
    if(req==="high_marketability_driver"&&drivers.some(d=>(d.marketability??0)>80))score+=12;
    if(req==="rookie_driver"&&drivers.some(d=>(d.age??99)<=21))score+=10;
    if(req==="national_driver")score+=4;
    if(req==="championship_contender"&&(team.reputation??0)>84)score+=14;
    if(req==="budget_team"&&(team.reputation??0)<78)score+=10;
    if(req==="top_3_team"&&(team.reputation??0)>86)score+=12;
  }
  ...
  for(const region of sponsor.preferredRegions??[])if(drivers.some(d=>d.fanbase?.regions?.[region]>60))score+=6;
  return clamp(score);
}
...
export function sendSponsorProposal(world, sponsorId) {
  ...
  const valuePerSeason=Math.round((sponsor.budget??500000)*(.22+interest/260)/100000)*100000;
  ...
  if(interest>=72){
    ...
    team.sponsorContracts.push({sponsorId:sponsor.id,valuePerSeason,duration:proposal.duration,expiresAt:world.currentSeason+proposal.duration,satisfaction:Math.min(88,interest),bonusConditions:sponsor.requirements??[]});
    ...
    return{ok:true,message:`${sponsor.name} aceptó la propuesta.`,proposal};
  }
...
export function processSponsors(world) {
  for(const team of world.teams??[]){
    for(const contract of team.sponsorContracts??[]){
      ...
      if(!sponsor)continue;
      const interest=evaluateSponsorInterest(sponsor,team,world);
      contract.satisfaction=clamp((contract.satisfaction??60)*.8+interest*.2);
